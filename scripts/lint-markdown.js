...
files.forEach((file) => {
  ...
  if (FILES_TO_IGNORE.some((fileToIgnore) => relativePath === fileToIgnore))
    return
  ...
  const isSeparatedEnglishCheck = FILES_TO_IGNORE_ENGLISH_CHECKS.some(
    (f) => relativePath === f
  )
  ...
  lines.forEach((line, index) => {
    ...
    const isTempMailSection =
      relativePath === 'docs/internet-tools.md' &&
